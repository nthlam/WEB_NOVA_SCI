// scripts/generate_uwb_locations_zone_1482_1941.js
...
const TOTAL_LOCATIONS = 1500;
const SESSION_COUNT = 18; // Number of different sessions
...
const TARGET_ZONE = {
  ...
  minY: 1840,
  maxY: 2300,
  name: "Zone 1482-1941 x 1840-2300"
};

// Calculate zone properties
const ZONE_WIDTH = TARGET_ZONE.maxX - TARGET_ZONE.minX; // 459
const ZONE_HEIGHT = TARGET_ZONE.maxY - TARGET_ZONE.minY; // 460
const ZONE_CENTER = {
  x: (TARGET_ZONE.minX + TARGET_ZONE.maxX) / 2, // 1711.5
  y: (TARGET_ZONE.minY + TARGET_ZONE.maxY) / 2  // 2070
};
...
const DISTRIBUTION_ZONES = [
  ...
  { 
    ...
    maxX: TARGET_ZONE.minX + ZONE_WIDTH * 0.45, 
    minY: TARGET_ZONE.minY + ZONE_HEIGHT * 0.55, 
  ...
  // Góc trên phải
  { 
    minX: TARGET_ZONE.minX + ZONE_WIDTH * 0.55, 
  ...
  { 
    ...
    maxY: TARGET_ZONE.minY + ZONE_HEIGHT * 0.45, 
    weight: 0.18, 
  ...
  // Vùng trung tâm
  { 
    minX: TARGET_ZONE.minX + ZONE_WIDTH * 0.30, 
    maxX: TARGET_ZONE.maxX - ZONE_WIDTH * 0.30, 
    minY: TARGET_ZONE.minY + ZONE_HEIGHT * 0.30, 
    maxY: TARGET_ZONE.maxY - ZONE_HEIGHT * 0.30, 
    weight: 0.16, 
    name: "Central Core" 
  }
];
...
const generateTimestamp = () => {
  ...
  const timeWeights = [
    { start: 0.0, end: 0.25, weight: 0.12 }, // 8:00-9:00 (12%)
    { start: 0.25, end: 0.5, weight: 0.38 }, // 9:00-10:00 (38%)
    { start: 0.5, end: 0.75, weight: 0.38 }, // 10:00-11:00 (38%)
    { start: 0.75, end: 1.0, weight: 0.12 }  // 11:00-12:00 (12%)
  ];
...
const generateSessionIds = (count) => {
  ...
  for (let i = 1; i <= count; i++) {
    sessions.push(`UWB_MID_${String(i).padStart(3, '0')}`);
  }
  return sessions;
};
...
const generateCartIds = (count) => {
  const carts = [];
  for (let i = 1; i <= count; i++) {
    carts.push(`CART_MID_${String(i).padStart(2, '0')}`);
  }
  return carts;
};
...
const generateCoordinates = () => {
  ...
  for (let i = 0; i < DISTRIBUTION_ZONES.length; i++) {
    ...
    if (random <= cumulativeWeight) {
      ...
      // Add some clustering effect towards zone center
      ...
      const noiseX = (Math.random() - 0.5) * 40; // ±20 units
...
const generateRawDistances = () => {
  ...
  for (let i = 0; i < anchorCount; i++) {
    // Random distance between 0.3m to 40m (in centimeters)
    const distance = 30 + Math.random() * 3970;
...
const generateUWBLocation = (sessionIds, cartIds) => {
  ...
  return {
    ...
    filtered: Math.random() > 0.11, // 89% filtered (good quality), 11% unfiltered
    tracking_mode: Math.random() > 0.16, // 84% in tracking mode, 16% not
...
// Main function
const generateMidZoneUWBLocations = async () => {
  try {
    const { startTime, endTime } = getTimeRange();
    console.log('🎯 Starting Mid-Zone UWB location generation...');
    ...
    console.log('\n🎉 Mid-Zone UWB location generation completed successfully!');
    ...
    console.log(`🔢 Generated ${TOTAL_LOCATIONS} records for comprehensive heatmap coverage`);
...
if (require.main === module) {
  generateMidZoneUWBLocations();
}

module.exports = { generateMidZoneUWBLocations };
