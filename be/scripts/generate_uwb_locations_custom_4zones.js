// scripts/generate_uwb_locations_custom_4zones.js
...
// 4 vị trí tập trung như yêu cầu: (2300,3333), (2333,2222), (2300,1222), và thêm vị trí thứ 4
const ZONES = [
  { centerX: 2300, centerY: 3333, weight: 0.30, radius: 300, name: "Zone 1" }, // 30% - tăng radius
  { centerX: 2333, centerY: 2222, weight: 0.25, radius: 250, name: "Zone 2" }, // 25% - tăng radius
  { centerX: 2300, centerY: 1222, weight: 0.25, radius: 250, name: "Zone 3" }, // 25% - tăng radius
  { centerX: 1800, centerY: 2800, weight: 0.20, radius: 200, name: "Zone 4" }  // 20% - tăng radius
];
...
const generateCoordinates = () => {
  ...
  for (let i = 0; i < ZONES.length; i++) {
    ...
    if (random <= cumulativeWeight) {
      ...
      // Add more variation to make it more realistic and spread out
      const jitter = (Math.random() - 0.5) * 120; // ±60 pixels - tăng jitter
...
const generateUWBLocation = (sessionIds, cartIds) => {
  ...
  return {
    ...
    filtered: Math.random() > 0.15, // 85% filtered (good quality), 15% unfiltered
    tracking_mode: Math.random() > 0.20, // 80% in tracking mode, 20% not
...
// Main function
const generateCustom4ZonesUWBLocations = async () => {
  try {
    const { startTime, endTime } = getTimeRange();
    console.log('📍 Starting Custom 4-Zones UWB location generation...');
    ...
    console.log('\n🎉 Custom 4-zones UWB location generation completed successfully!');
...
if (require.main === module) {
  generateCustom4ZonesUWBLocations();
}

module.exports = { generateCustom4ZonesUWBLocations };
